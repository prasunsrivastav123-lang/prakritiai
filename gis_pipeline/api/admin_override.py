...
ROADS_GDF = gpd.read_parquet("data/processed/roads.parquet")

@router.post("/api/admin/manual-hazard")
async def ingest_manual_hazard(hazard: ManualHazardModel):
    ...
    try:
        ...
        if nearest_roads.empty or pd.isna(nearest_roads.iloc[0].get("edge_id")):
            raise HTTPException(status_code=404, detail="No road found within 500m of the provided coordinates.")
        ...
        # from realtime.replanner import replanner_instance
...
@router.post("/api/admin/close-road")
